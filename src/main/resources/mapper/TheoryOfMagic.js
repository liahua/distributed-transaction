// ==UserScript==
// @name         aardvark arcanum auto - Harrygiel's fork, mod for 0.8.1
// @version      1716
// @author       aardvark, Linspatz, Harrygiel, Lucibear
...
function tc_populate_spells() {
    ...
    for (let qs of document.querySelectorAll(".spells .bottom .spellbook table tr")) {
        if (qs.childElementCount == 3) {
            var spell = qs.children[1].innerHTML.toLowerCase().trim();
...
function tc_populate_resources() {
    var resources = document.querySelectorAll("div.game-main div.res-list div.res-group div.rsrc.separate.hidable");
    if (resources.length == 0) {
        resources = document.querySelectorAll("div.game-main div.res-list div");
    }
    for (let n of resources) {
        var name = n.querySelector("span.item-name").innerText.toLowerCase();
        var vals = n.querySelectorAll("span.num-align")[0].innerText.split("/");
...
function tc_populate_bars() {
    for (let n of document.querySelectorAll("div.game-main div.vitals div.statbars div.hidable.statbar")) {
        var name = n.querySelector("span.name").innerText.toLowerCase();
        var vals = n.querySelectorAll("span.barspan")[0].innerText.split("/");
...
function tc_populate_actions() {
    ...
    for (let qs of document.querySelectorAll(".main-tasks .task-list .task-btn.hidable:not(.locked):not(.disabled)")) {
...
function tc_automate() {
    ...
    if (tc_check_resource("herbs", 1) && !tc_check_resource("gold", 1)) {
        var missing = (tc_resources.get("gold")[1] - tc_resources.get("gold")[0]);
        var sellamount = Math.min(missing / 2, tc_resources.get("herbs")[1] / 2);
...
function tc_selljunk() {
    ...
    function checkmatch(m) {
        for (let i of sell_match) if (RegExp(i).test(m)) return true;
        return false;
    }
...
function tc_autofocus() {
    ...
    for (let qs of document.querySelectorAll(".skills .skill")) {
        ...
        var btn = qs.querySelectorAll("button")[0];
        var text = btn.innerHTML.trim();	// Can be Unlock, Train, Stop
        if (text == "s" || btn.disabled) continue;
...
function updateResourceColor() {
    let r = document.querySelectorAll("div.game-main div.res-list div.res-group div.rsrc.separate.hidable");
    for (let q of r) {
        var values = q.querySelectorAll("span.num-align")[0].innerText.split("/");
        var pct = values[0] / values[1];
        if (values[0] === values[1]) {
            q.style.color = '#00b800';
        } else if (values[0] === 0) {
            q.style.color = '#FF0000';
        } else if (pct >= 0.9) {
            q.style.color = '#80b800';
        } else if (pct <= 0.1) {
            q.style.color = '#ff8000';
        } else {
            q.style.color = '#ffa500';
        }
    }
}
